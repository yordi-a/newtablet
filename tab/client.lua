...
RegisterNetEvent('yordi:tablet')
AddEventHandler('yordi:tablet', function()
    startAnim()
    exports['progressBars']:startUI(7500, "Tablet başlatılıyor!")
    Citizen.Wait(7500)
    exports['mythic_notify']:DoHudText('yordi', 'Tablet açıldı')
    tabEnabled = not tabEnabled
    REQUEST_NUI_FOCUS(tabEnabled)
    print("/tablet yazarak birisi tableti kullandı!")
    Citizen.Wait(0)
end)
...
RegisterNUICallback(
    ...
    function(data)
        -- Do tablet hide shit
        if data.load then
            print("Yordi Tablet: Yenilendi")
            tabLoaded = true
        elseif data.hide then
            print("Yordi Tablet: Kapatıldı")
            stopAnim()
            SetNuiFocus(false, false) -- Don't REQUEST_NUI_FOCUS here
            tabEnabled = false
            exports['mythic_notify']:DoHudText('error', 'Tablet kapatıldı')
...
Citizen.CreateThread(
    function()
        ...
        if timeout == true then
            print("Yordi Tablet: Yenilemede hata")
        -- return ---- Quit
        end
        ...
        while true do
           ...
           -- if (IsControlJustPressed(0, 58)) and GetLastInputMethod( 0 ) then
